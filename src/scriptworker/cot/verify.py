...
class LinkOfTrust(object):
    ...
    def _set(self, prop_name, value):
        ...
        if prev is not None:
            raise CoTError("LinkOfTrust {}: re-setting {} to {} when it is already set to {}!".format(str(self.name), prop_name, value, prev))
...
def verify_docker_image_sha(chain, link):
    ...
    if isinstance(image, dict) and image["type"] == "task-image":
        ...
        if docker_image_task_id != image["taskId"]:
            errors.append(
                "{} {} docker-image taskId isn't consistent!: {} vs {}".format(
                    link.name, link.task_id, docker_image_task_id, task["payload"]["image"]["taskId"]
                )
            )
        else:
            path = image["path"]
        ...
        if prebuilt_task_types != "any" and link.task_type not in prebuilt_task_types:
            errors.append("Task type {} not allowed to use a indexed docker image!".format(link.task_type))
...
def find_sorted_task_dependencies(task, task_name, task_id):
    ...
    cot_input_dependencies = [
        _craft_dependency_tuple(task_name, task_type, task_id) for task_type, task_id in task["extra"].get("chainOfTrust", {}).get("inputs", {}).items()
    ]
        ...
        for seed in verify_key_seeds:
            try:
                ...
                verify_ed25519_signature(
                    ...
                    binary_contents,
                    signature,
                    "{} {}: {} ed25519 cot signature doesn't verify against {}: %(exc)s".format(link.name, link.task_id, link.worker_impl, seed),
                )
            ...
            errors = errors or [
                "{} {}: Unknown error verifying ed25519 cot signature. worker_impl {} verify_keys {}".format(
                    link.name, link.task_id, link.worker_impl, verify_key_seeds
                )
            ]
            message = "\n".join(errors)
            raise CoTError(message)
    link.cot = load_json_or_yaml(
        unsigned_path, is_path=True, exception=CoTError, message="{} {}: Invalid unsigned cot json body! %(exc)s".format(link.name, link.task_id)
    )
...
def _take_expires_out_from_artifacts_in_payload(payload):
    ...
    if artifacts is None:
        ...
        raise CoTError('Unsupported type of artifacts. Found: "{}". Expected: dict, list or undefined. Payload: {}'.format(type(artifacts), payload))
...
def verify_link_in_task_graph(chain, decision_link, task_link):
    ...
    """
    log.info(
        "Verifying the {} {} task definition is part of the {} {} task graph...".format(
...
async def _get_additional_github_releases_jsone_context(decision_link):
    ...
    return {
        "event": {
            ...
            "sender": {"login": release_data["author"]["login"]},
        }
    }
...
async def _get_additional_git_cron_jsone_context(decision_link):
    ...
    return {
        ...
        "event": {
            ...
            "release": {"tag_name": revision, "target_commitish": branch, "published_at": get_push_date_time(task, source_env_prefix)},
            "sender": {"login": user},
        },
...
async def _get_additional_github_pull_request_jsone_context(decision_link):
    ...
    return {
        "event": {
            ...
            "sender": {"login": pull_request_data["head"]["user"]["login"]},
        }
    }
...
async def verify_parent_task_definition(chain, parent_link):
    ...
    try:
        ...
        msg = "Error while rebuilding {} {} task definition!".format(parent_link.name, parent_link.task_id)
        log.exception(msg)
        raise CoTError(msg + "\n{}".format(str(e)))
...
def check_and_update_action_task_group_id(parent_link, decision_link, rebuilt_definitions):
    ...
    if rebuilt_gid not in acceptable_gids:
        raise CoTError("{} ACTION_TASK_GROUP_ID {} not in {}!".format(parent_link.name, rebuilt_gid, acceptable_gids))
...
async def trace_back_to_tree(chain):
    ...
    if len(set(repos.values())) > 1:
        for obj, repo in repos.items():
            if obj.decision_task_id == chain.decision_task_id:
                if repo != my_repo:
                    errors.append("{} {} repo {} doesn't match my repo {}!".format(obj.name, obj.task_id, repo, my_repo))
            ...
            #  is used for a release-priv task, for example.)
                ...
                errors.append("{} {} has no privileged repo on an restricted privilege scope!".format(obj.name, obj.task_id))
    ...
    if restricted_privs and await chain.is_try_or_pull_request():
        errors.append("{} {} has restricted privilege scope, and is_try_or_pull_request()!".format(chain.name, chain.task_id))
    raise_on_errors(errors)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for upstream_artifact in context.task["payload"]["upstreamArtifacts"]:
            ...
            for path in upstream_artifact["paths"]:
                ...
                await download_artifacts(context, [url], parent_dir=parent_dir, valid_artifact_task_ids=[task_id])
